...
class CoreParser {
    ...
    async copyRes(tempPath) {
        const i = await fs.pathExists(tempPath + '/res')
        if (i) {
            logger.greate('拷贝资源文件')
            await fs.ensureDir(this.output + '/res')
            fs.copy(tempPath + '/res', this.output + '/res')
        }
    }
