    ...
    FunctionDeclaration: {
      ...
      exit(path) {
        if (isTemplate) {
          ...
          const openning = t.jsxOpeningElement(t.jSXIdentifier('template'), [makeJSXAttribute('name', `${funName}`)])
        ...
        specifiers.forEach(sp => {
          ...
          if (source.indexOf('./') >= 0) {
            ...
            if (real_node_modules_path) {
              const relatedPath = '/nodeModules' + real_node_modules_path.replace(_path.resolve('./node_modules'), '')
  ...
  path.node.attributes.forEach(arrt => {
    ...
    if (WXML_event[attrName]) {
      ...
      if (t.isCallExpression(arrt.value.expression) || t.isArrowFunctionExpression(arrt.value.expression)) {
        ...
        logger.warn(`警告：小程序官方目前不支持事件中使用function/arrow function，因此 '${warningCode}' 不会被编译`)
      }
